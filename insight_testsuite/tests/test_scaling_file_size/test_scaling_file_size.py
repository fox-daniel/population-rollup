...
def main():
    ...
    with open(path_to_log, "w", newline="") as logfile:
        ...
        logwriter.writerow([f"****Measure how pipeline.py scales with the number of rows in the input csv.****"])
        logwriter.writerow(["Returned from test_scaling_file_size.py at {datetime.now().isoformat(timespec='seconds')}"])
